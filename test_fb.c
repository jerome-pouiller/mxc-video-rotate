 ...
 * Adapted by Jérôme Pouiller <j.pouiller@sysmic.fr>
 *
...
#define LINE_LEN finfo.line_length
#define XRES vinfo.xres
#define YRES vinfo.yres
//#define LINE_LEN (240 * 2)
//#define XRES 240
//#define YRES 320
...
void put_pixel(int x, int y, int c)
{
    ...
    unsigned int pix_offset = (x * 2) + y * LINE_LEN;

    // offset by the current buffer start
    pix_offset += cur_page * page_size;

    // now this is about the same as 'fbp[pix_offset] = value'
    *((char *)(fbp + pix_offset)) = c & 0xFF;
    *((char *)(fbp + pix_offset + 1)) = c >> 8;
}
